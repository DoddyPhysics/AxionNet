	...
	def __init__(self,fname='configuration_card.ini',ifsampling=False,mnum=None,hypervec=None):
		...
		if ifsampling:
			...
			if self.modnum == 1:
				...
				beta = config.getfloat('Hyperparameter', 'Dimension')
				self.hyper=np.vstack((nax,kmin,kmax,mmin,mmax,beta))
			elif self.modnum == 3:
				...
				Imax = config.getint('Hyperparameter','Imax')
				Nvolmax = config.getint('Hyperparameter','Nvolmax')
				flag = config.getint('Hyperparameter','Flag')
				a0 = config.getfloat('Hyperparameter','a0')
				self.hyper=np.vstack((nax,F,Lambda,smin,smax,Imax,Nvolmax,flag,a0))
			elif self.modnum == 4:
				c = config.getfloat('Hyperparameter','Dimension')
				a0 = config.getfloat('Hyperparameter','a0')
				b0 = config.getfloat('Hyperparameter','b0')
...
#######################################################	#######################################################	
	...
	def getParams(self):
		...
		if mo == 2:
			...
			k = (np.random.uniform(kmin,kmax,(n,L))) #random matrix k from log normal distribution
			kk = np.exp(-k)
			kT = kk.transpose() # transpose of random matrix k
			k2 = np.dot(kk,kT)/L  # Construction of symmeterised Kahler matric for real axion fields
			ev,pT = np.linalg.eigh(k2) # calculation of eigen values and eigen vectors
			fef = np.sqrt(2*ev)
			fef2=np.log(fef)
			p = pT.transpose() # tranpose of rotational matrix constructed of eigen vectors
